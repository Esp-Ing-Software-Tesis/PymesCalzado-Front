...
export class TasksService {
  ...
  // Servicio Detalle de tareas
  getTaskDetail(taskId: number): Observable<TaskDetailsMock[]> {
    const taskDetailMock: TaskDetailsMock[] = this.setMockTaskDetailOptions();
    const filteredTaskDetail = taskDetailMock.filter((task) => task.task_id === taskId);
    /*return throwError(() => ({
    }));*/
    return of(filteredTaskDetail);
  }
  ...
  getTasksByOperator(operatorDocument: number): Observable<TasksByOperator[]> {
    const tasksByOperatorMock: TasksByOperator[] = this.setMockTasksByOperator();
    const filteredTasksByOperator = tasksByOperatorMock.filter((task) => task.operator_document === operatorDocument);
    /*return throwError(() => ({
        }));*/
    return of(filteredTasksByOperator);
  }
  ...
  private setMockTaskDetailOptions(): TaskDetailsMock[] {
    const taskDetail: TaskDetailsMock[] = [
      ...
      // --- 3: En progreso (solo finished_at null) ---
      {
        task_id: 3,
        LineasProduccion: [
          {
      ...
      // --- 4: Nuevo (todas fechas null, estado Nuevo) ---
      {
        task_id: 4,
        LineasProduccion: [
          {
      ...
      // --- 7: Ya en Guarnicion (Corte finalizado) ---
      {
        task_id: 7,
        LineasProduccion: [
          {
      ...
      // --- 8: Ya en Solador (Corte y Guarnicion finalizados) ---
      {
        task_id: 8,
        LineasProduccion: [
          {
      ...
      // --- 9: Ya en Emplantillado (todas las anteriores finalizadas) ---
      {
        task_id: 9,
        LineasProduccion: [
          {
      ...
      // --- 10: En Termofijado (con líneas opcionales) ---
      {
        task_id: 10,
        LineasProduccion: [
          {
      ...
      // --- 13: En Solador, todo anterior finalizado ---
      {
        task_id: 13,
        LineasProduccion: [
          {
      ...
      // --- 15: En Emplantillado (todo anterior finalizado) ---
      {
        task_id: 15,
        LineasProduccion: [
          {
